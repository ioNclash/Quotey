...
chown pi:pi "$QUOTES_FILE" "$CURRENT_QUOTE_FILE" #Change 'pi:pi' to your desired user and group
chmod 644 "$QUOTES_FILE" "$CURRENT_QUOTE_FILE" #Set permissions to read and write for owner, allows others to read
...
(crontab -l 2>/dev/null; echo "0 0 * * * python3 /home/pi/Quotey/quotey.py") | crontab -
echo "Cron job added to update quote daily at midnight."
(crontab -l 2>/dev/null; echo "@reboot cd /home/pi/Quotey && python3 -m gunicorn --bind 0.0.0.0:5000 api:app") | crontab -
